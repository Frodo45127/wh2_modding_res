...
* ***`Mayor`***: Capture zones that show up in the minimap and in the top of the screen, as unique capture points.
...
Towers are.... well, you know the drill. Copy them from the example map, then put them into your map, replacing the dummy towers, and remember to assign them their `Capture Zones`. You'll see some ***white arrows*** coming out of the tower. Those are the `Projectile Emitters`. Same thing as with the rest, we cannot create then, and these require tinkering with the layer files of the map to set them up. A complex mess. So just copy the example towers, then move the emitters around until you have them where you want. That's all. 
